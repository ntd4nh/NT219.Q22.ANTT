...
Đề tài hướng dẫn sinh viên thiết kế, triển khai và đánh giá **một hệ thống API‑first cho dịch vụ công ty nhỏ** (ví dụ: SaaS B2B nhỏ, e‑commerce microservice, hoặc internal admin API), tập trung vào:
...
**Giả thuyết (đo được):** So với baseline chưa hardening đầy đủ, stack gồm API Gateway (rate limit + WAF + JWT validation), IdP (OIDC), short‑lived tokens, KMS cho secrets và detection rules sẽ làm giảm tỷ lệ request tấn công thành công trong bộ test D1-D4 và giữ độ trễ p95 ở mức chấp nhận được cho SME.
...
* **Mô tả:** long‑lived access/refresh tokens stolen (XSS, insecure storage), hoặc leaked API keys.
* **Mitigation:** short‑lived access tokens, PKCE, rotate refresh tokens, refresh token rotation, denylist ngắn hạn cho incident response, device binding (hướng mở rộng).
...
6. **Automated testing:** run SAST (Bandit/Brakeman/ESLint), DAST (OWASP ZAP), API fuzzing (fuzzapi, restler), và BOLA test suites.
...
Tiêu chí pass đề xuất cho lab:
- D1-D4 có log chứng cứ đầy đủ (request, response code, trace id).
- Tỷ lệ request tấn công thành công ở bản hardened thấp hơn baseline theo từng kịch bản.
- Có báo cáo trade-off bảo mật/độ trễ/chi phí.
...
1. **Mid‑term:** architecture diagram, threat model, references, literature survey sketch, goals, demonstration proposal.
...
Ghi chú bám NT219:
- Có tối thiểu 1 bài báo mới (2020+) liên quan trực tiếp chủ đề để thảo luận giải pháp.
- Tách rõ kết quả nghiên cứu (research results) và kết quả demo (demonstration results) ở báo cáo cuối kỳ.
